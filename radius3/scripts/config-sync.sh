...
# ============================================================================
# Config Sync Script for FreeRADIUS radius3
...
# Main sync function
main_sync() {
    log "Starting configuration synchronization for radius3..."
    ...
    sync_config "freeradius/server/radius3/radiusd.conf" "/etc/raddb/radiusd.conf" && ((configs_synced++))
    sync_config "freeradius/server/radius3/sql.conf" "/etc/raddb/mods-available/sql" && ((configs_synced++))
    sync_config "freeradius/server/radius3/clients.conf" "/etc/raddb/clients.conf" && ((configs_synced++))
...
# Main loop
main() {
    log "Config Sync started for radius3"
