...
# FreeRADIUS Production Configuration Loader for Windows
...
Write-Host "FreeRADIUS Production Configuration Loader for ETCD v3" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host
...
function Load-ConfigToEtcd {
    ...
    if (Test-Path $FilePath) {
        try {
            ...
            if ($content) {
                $base64Key = Convert-ToBase64 $Key
                $base64Value = Convert-ToBase64 $content
...
function Load-ServiceConfigs {
    ...
    $serviceDir = Join-Path $PRODUCTION_CONFIG_DIR $ServiceName
    ...
    if (Test-Path $modsConfigDir) {
        Get-ChildItem $modsConfigDir -File -Recurse | ForEach-Object {
            $relativePath = $_.FullName.Substring($serviceDir.Length + 1)
            $etcdKey = "/freeradius/$ServiceName/mods-config/$relativePath"
            Load-ConfigToEtcd -Key $etcdKey -FilePath $_.FullName
        }
    }
...
# Main execution
Write-Host "Starting production configuration loading process..." -ForegroundColor Yellow
Write-Host "ETCD URL: $ETCD_BASE_URL" -ForegroundColor Yellow
Write-Host "Production Config Directory: $PRODUCTION_CONFIG_DIR" -ForegroundColor Yellow
Write-Host
...
if (-not (Test-Path $PRODUCTION_CONFIG_DIR)) {
    Write-Host "❌ Production config directory not found: $PRODUCTION_CONFIG_DIR" -ForegroundColor Red
    exit 1
}

Write-Host "Loading production configurations..." -ForegroundColor Yellow
...
Write-Host
Write-Host "Production configuration loading completed!" -ForegroundColor Green
Write-Host
