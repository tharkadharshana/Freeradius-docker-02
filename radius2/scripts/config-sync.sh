...
# ============================================================================
# Config Sync Script for FreeRADIUS radius2
...
# Main sync function
main_sync() {
    log "Starting configuration synchronization for radius2..."
    ...
    sync_config "freeradius/server/radius2/radiusd.conf" "/etc/raddb/radiusd.conf" && ((configs_synced++))
    sync_config "freeradius/server/radius2/sql.conf" "/etc/raddb/mods-available/sql" && ((configs_synced++))
    sync_config "freeradius/server/radius2/clients.conf" "/etc/raddb/clients.conf" && ((configs_synced++))
...
# Main loop
main() {
    log "Config Sync started for radius2"
